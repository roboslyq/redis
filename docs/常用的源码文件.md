...
- ae.c
  - redis I/O模型实现，是统一入口。包装了下面这些具体的不同实现。
- ae_epoll.c
  - epoll I/O模型实现
- ae_evport.c
  - evport I/O模型实现
...
- dict.c 
  - 字典dict数据结构实现，主要对于内存中的hash进行管理。在redis中， 字典的作用是非常巨大的，对Redis数据库的增删改查等操作都构建在对字典的操作之上。因此，我将dict.c归于db，而不是data Struct结构。
